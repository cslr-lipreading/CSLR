...
def main(rank, args):
    ...
    if args.distributed:
        ...
        torch.distributed.init_process_group(backend=args.backend, init_method='env://', world_size=args.world_size,
                                             rank=args.rank)
    ...
    if args.mode == "training":
        ...
        model.fit(
            ...
            epochs=getattr(args.config, "epochs", 300),
    ...
    # Stochastic Weight Averaging
        ...
        model.swa(dataset_train, callback_path=args.config.callback_path,
                  start_epoch=args.swa_epochs[0] if args.swa_epochs else None,
                  end_epoch=args.swa_epochs[1] if args.swa_epochs else None, epochs_list=args.swa_epochs_list,
                  update_steps=args.steps_per_epoch, swa_type=args.swa_type, precision=args.config.precision)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config_file", type=str, default="configs/LRS23/VO/GRU.py",
                        help="Python configuration file containing model hyperparameters")
    parser.add_argument("-m", "--mode", type=str, default="training",
                        help="Mode : training, evaluation, swa, pass, eval_time")
    parser.add_argument("-i", "--checkpoint", type=str, default=None, help="Load model from checkpoint name")
    parser.add_argument("-j", "--num_workers", type=int, default=16, help="Number of data loading workers")
    ...
    parser.add_argument("-d", "--distributed", action="store_true", default=False,
                        help="Distributed data parallelization")
    parser.add_argument("--parallel", action="store_true", default=True,
                        help="Parallelize model using data parallelization")
    ...
    parser.add_argument("--saving_period_epoch", type=int, default=100, help="Model saving every 'n' epochs")
    ...
    parser.add_argument("--batch_size_eval", type=int, default=1, help="Evaluation batch size")
    ...
    parser.add_argument("--detect_anomaly", action="store_true",
                        help="Enable or disable the autograd anomaly detection")
