...
class CTCGreedySearchDecoder(nn.Module):
    ...
    def __init__(self, tokenizer_path, blank_token=[0, 1, 2], mode=None):
        ...
        self.vocal = {}
        if mode in ['en_cn']:
            f = open('./data_process/new/en_cn_voc.txt', 'r')
            data = f.read().split('\n')
            f.close()
            for _ in data:
                sub, id = _.split('\t')
                self.vocal[int(id)] = sub
        elif mode in ['en']:
            f = open('./data_process/new/en_voc.txt', 'r')
            data = f.read().split('\n')
            f.close()
            for _ in data:
                sub, id = _.split('\t')
            ...
            f = open('./data_process/new/cn_voc.txt', 'r')
            data = f.read().split('\n')
            f.close()
            for _ in data:
                sub, id = _.split('\t')
    ...
    def forward(self, outputs, from_logits=True):
        ...
        for b in tokens:
            # print(b)
            i = [self.vocal[_] for _ in b]
            i = ''.join(i)
            # print('join i:', i)

            i = i.replace(' ', '').replace('▁', ' ')
            # print('join i.replace:', i)
            while len(i):
                if i[0] == ' ':
                    i = i[1:]
                else:
            ...
            while len(i):
                if i[-1] == ' ':
                    ...
                    break
            # print('remove start end black i:', i)
            t.append(i)
        # print(t)
        return t
    ...
    def greedy_search(self, logits, logits_len):
        ...
        for b in range(logits.size(0)):
            ...
            # print('preds_b:', preds_b)
            # Remove Consecutives
            preds_b = preds_b.unique_consecutive(dim=-1)
            # print('preds_b.unique_consecutive(dim=-1):', preds_b)

            # Remove Blanks
            preds_b = [token.item() for token in preds_b if token not in self.blank_token]
...
class CTCBeamSearchDecoder(Module):
    ...
    def forward(self, outputs, from_logits=True):
        ...
        # print(tokens)
        # print(len(tokens))
        # print([len(_) for _ in tokens])
        # [[138, 42, 140, 150, 138, 17, 45, 164, 147, 7, 145, 141, 49], [70, 304, 334, 4, 38, 164, 0, 0, 0, 0, 0, 0, 0], [19, 141, 152, 68, 13, 140, 183, 234, 0, 0, 0, 0, 0], [5, 129, 17, 11, 13, 10, 0, 0, 0, 0, 0, 0, 0], [138, 185, 219, 157, 7, 10, 154, 16, 158, 0, 0, 0, 0], [70, 304, 334, 4, 38, 164, 0, 0, 0, 0, 0, 0, 0], [138, 251, 321, 58, 144, 141, 159, 84, 145, 0, 0, 0, 0], [61, 66, 56, 290, 318, 0, 0, 0, 0, 0, 0, 0, 0], [61, 66, 56, 290, 318, 0, 0, 0, 0, 0, 0, 0, 0], [63, 30, 29, 8, 149, 31, 147, 26, 150, 0, 0, 0, 0], [63, 30, 29, 8, 149, 31, 147, 26, 150, 0, 0, 0, 0], [69, 208, 239, 227, 0, 0, 0, 0, 0, 0, 0, 0, 0], [138, 294, 189, 308, 307, 0, 0, 0, 0, 0, 0, 0, 0], [138, 185, 219, 157, 7, 10, 154, 16, 158, 0, 0, 0, 0], [63, 30, 29, 8, 149, 31, 147, 26, 150, 0, 0, 0, 0], [138, 176, 163, 152, 142, 43, 33, 146, 52, 150, 145, 0, 0]]
        # 16
        # [10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10]# 全部为最大值
        print('be')
        print(tokens)
        re = self.tokenizer.decode(tokens)
        print('af')
        return re
