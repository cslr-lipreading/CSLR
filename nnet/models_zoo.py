...
class VisualEfficientCTC(Model):

    def __init__(self, vocab_size=256, interctc_blocks=[3, 6, 9], test_augments=None, my_settings=None):
        super(VisualEfficientCTC, self).__init__(name="Visual Efficient Inter CTC")

        self.my_settings = my_settings
        self.encoder = networks.VisualEfficientEncoder(vocab_size=vocab_size, interctc_blocks=interctc_blocks, my_settings=my_settings)
    ...
    def forward(self, inputs):
        ...
        video, video_lengths, _ = inputs# _ is the name of sample
        video = video.reshape(-1, *video.shape[2:])
        video_lengths = video_lengths.reshape(-1)
    ...
    def compile(
        ...
        super(VisualEfficientCTC, self).compile(
            losses=losses,
