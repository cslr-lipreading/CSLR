...
import math
import numpy as np
from nnet.lipreading.models.resnet import ResNet, BasicBlock
from nnet.lipreading.models.resnet1D import ResNet1D, BasicBlock1D
from nnet.lipreading.models.shufflenetv2 import ShuffleNetV2
from nnet.lipreading.models.tcn import MultibranchTemporalConvNet, TemporalConvNet
from nnet.lipreading.models.densetcn import DenseTemporalConvNet
...
class MultiscaleMultibranchTCN(nn.Module):
    def __init__(self, input_size, num_channels, tcn_options, dropout, relu_type, dwpw=False):
        ...
        # self.tcn_output = nn.Linear(num_channels[-1] * 2, num_channels[-1])

        # self.consensus_func = _average_batch

    def forward(self, x, l):
        # x needs to have dimension (N, C, L) in order to be passed into CNN
        # print('x.shape', x.shape)# x.shape torch.Size([24, 90, 128])
        xtrans = x.transpose(1, 2)
        out = self.mb_ms_tcn(xtrans).transpose(1, 2)
        # print('out.shape', out.shape)# out.shape torch.Size([24, 90, 128])
