...
class MyDataset(object):
    ...
    def _get_files_for_partition(self):
        ...
        self._data_files = [ f.replace('\\', '/') for f in self._data_files if f.replace('\\', '/').split('/')[self.label_idx] in self._labels ]
    ...
    def _apply_variable_length_aug(self, filename, raw_data):
        ...
        info_txt = os.path.join(self._annonation_direc, *filename.replace('\\', '/').split('/')[self.label_idx:] )  # swap base folder
