...
class Model(Module):
    ...
    def distribute_strategy(self, rank, sync_batch_norm=True):
        if sync_batch_norm:
            object.__setattr__(self, "ddp", torch.nn.parallel.DistributedDataParallel(
                norms.SyncBatchNorm.convert_sync_batchnorm(self), device_ids=[rank]))
        else:
        ...
        # Dictionary
            ...
            for key in struct:
                if not key in outputs:
                    raise Exception(
                        "Found unexpected dict key: {}. Valid output names are: {}".format(key, outputs.keys()))
    ...
    def forward_model(self, inputs, targets, compute_metrics=True, verbose=0, mode='train'):
        # print(inputs.shape)
        # print(targets)
        _, __, ___, ____ = targets
        # print(_.shape, __.shape, ___.shape)
        # torch.Size([16, 3, 20]) torch.Size([16, 3]) torch.Size([16, 3, 1])
        targets = (_.reshape(-1, *_.shape[2:]), __.reshape(-1), ___.reshape(-1, *___.shape[2:]), ____.reshape(-1, *____.shape[2:]))
        ...
        for key in outputs:
            ...
            if self.losses[key] != None:
                ...
                targets[key] = (_.reshape(-1, 1, _.shape[-1])[:, 0, :], __, ___, ____)
    ...
    def train_step(self, inputs, targets, precision, grad_scaler, accumulated_steps, acc_step, eval_training):
        ...
        if "cuda" in str(self.device):
            with torch.cuda.amp.autocast(enabled=precision != torch.float32, dtype=precision):
                batch_losses, batch_metrics, batch_truths, batch_preds = self.forward_model(inputs, targets,
                                                                                            compute_metrics=eval_training, mode='train')
        else:
            batch_losses, batch_metrics, batch_truths, batch_preds = self.forward_model(inputs, targets,
                                                                                        compute_metrics=eval_training, mode='train')
        ...
        if self.grad_max_norm != None:
            if not sum(v.item() for v in grad_scaler._found_inf_per_device(self.optimizer).values()):  # nan grad
                self.add_info("grad_norm",
                              round(torch.nn.utils.clip_grad_norm_(self.parameters(), self.grad_max_norm).item(), 4))
        ...
        if "cuda" in str(self.device) and self.model_step % 100 == 0:
            self.add_info("{}_memory".format(str(self.device)), round(
                100 * torch.cuda.memory_reserved(self.device) / torch.cuda.get_device_properties(
    ...
    def show_dict(self, module=None):
        ...
        for id, (key, value) in enumerate(state_dict.items()):
            print(
                "{} {} type: {:<12} numel: {:<12} shape: {:<20} mean: {:<12.4f} std: {:<12.4f} min: {:<12.4f} max: {:<12.4f} dtype: {:<12} device: {}".format(
                    str(id) + " " * (max_len_id - len(str(id))), key + " " * (max_len_key - len(key)),
                    "param" if isinstance(value, nn.Parameter) else "buffer", value.numel(), str(tuple(value.size())),
                    value.float().mean(), value.float().std(), value.float().min(), value.float().max(),
                    str(value.dtype).replace("torch.", ""), str(value.device)))
    ...
    def show_modules(self, module=None):
        ...
        for id, (key, value) in enumerate(named_modules.items()):
            print("{} {} class: {} device: {}".format(str(id) + " " * (max_len_id - len(str(id))),
                                                      key + " " * (max_len_key - len(key)),
                                                      type(value).__name__ + " " * (
                                                                  max_len_class - len(type(value).__name__)),
                                                      value.device if hasattr(value, "device") else ""))

    def plot_params(self):

        for param_name, param in self.named_parameters():
            plt.title("{} \n shape: {} / mean: {:.4f} / std: {:.4f}".format(param_name, str(tuple(param.size())),
    ...
    def save(self, path, save_optimizer=True):
        ...
        torch.save({
            ...
            "optimizer_state_dict": None if not save_optimizer else {key: value.state_dict() for key, value in
                                                                     self.optimizer.items()} if isinstance(
    ...
    def load(self, path, load_optimizer=True, verbose=True, strict=True):
        ...
        if checkpoint["is_distributed"] and not self.is_distributed:
            self.load_state_dict(
                {key.replace(".module.", "."): value for key, value in checkpoint["model_state_dict"].items()},
                strict=strict)
        else:
    ...
    def on_epoch_end(self, evaluate, save, log_figure, callback_path, epoch, inputs, targets, dataset_eval, eval_steps,
                     verbose_eval, writer, recompute_metrics):
        print('on epoch end')


        self.on_step_end(evaluate, save, log_figure, callback_path, epoch, epoch, inputs, targets, dataset_eval,
                         eval_steps, verbose_eval, writer, recompute_metrics, tag="epoch")
    ...
    def on_step_end(self, evaluate, save, log_figure, callback_path, epoch, step, inputs, targets, dataset_eval,
                    eval_steps, verbose_eval, writer, recompute_metrics, tag="step"):

        # Evaluate Model
        if evaluate:
            self._evaluate(dataset_eval, writer, step, eval_steps, verbose_eval, recompute_metrics,
                           tag="Evaluation-" + tag)
            self.train()
    ...
    def print_step(self, losses, metrics, tag):
        ...
        # val metrics
        # for key, value in metrics.items():
        #     s = "{} {}: {:.4f}".format(tag, key, value)
        #     print(s)
        #     self.print_result.append(s)
    ...
    def fit(self, dataset_train, epochs, dataset_eval=None, eval_steps=None, verbose_eval=0, initial_epoch=0,
            callback_path=None, steps_per_epoch=None, precision=torch.float32, accumulated_steps=1,
            eval_period_step=None, eval_period_epoch=1, saving_period_step=None, saving_period_epoch=1,
            log_figure_period_step=None, log_figure_period_epoch=1, step_log_period=10, eval_training=True,
            dist_log=False, grad_init_scale=65536.0, detect_anomaly=False, recompute_metrics=False):
        ...
        self.grad_scaler = torch.cuda.amp.GradScaler(init_scale=grad_init_scale, enabled=(grad_init_scale != None) and (
                    precision == torch.float16) and ("cuda" in str(self.device)))
        ...
        try:
            ...
            for epoch in range(initial_epoch, epochs):
                ...
                if self.rank == 0:
                    ...
                    epoch_iterator = tqdm(dataset_train,
                ...
                for step, batch in enumerate(epoch_iterator):
                    ...
                    batch_losses, batch_metrics, acc_step = self.train_step(inputs=inputs, targets=targets,
                                                                            precision=precision,
                    ...
                    for key, value in batch_losses.items():
                        epoch_losses[key] = epoch_losses[
                                                key] + value.detach() if key in epoch_losses else value.detach().type(
                            torch.float64)
                    for key, value in batch_metrics.items():
                        epoch_metrics[key] = epoch_metrics[
                                                 key] + value.detach() if key in epoch_metrics else value.detach().type(
                            torch.float64)
                    ...
                    if writer is not None and self.model_step % step_log_period == 0:
                        self.log_step(batch_losses, batch_metrics, self.infos, writer, self.model_step, "Training-step",
                                      dist_log)

                    # On Batch End
                    # print(self.model_step % saving_period_step == 0 if saving_period_step != None else False)
                    self.on_step_end(
                    ...
                    if steps_per_epoch is not None:
                        if step + 1 >= steps_per_epoch * accumulated_steps:
                            ...
                            break
                    # print(step)
                    # print(steps_per_epoch * accumulated_steps)
                ...
                for key, value in epoch_losses.items():
                    epoch_losses[key] = value / (
                        steps_per_epoch * accumulated_steps if steps_per_epoch is not None else len(dataset_train))
                ...
                torch.save(self.encoder.state_dict(), '{}/pt/model_{}.pt'.format(self.my_settings['save_path'], self.epoch - 1))
        ...
        # Exception Handler
            ...
            if self.rank == 0 and writer is not None:
                writer.add_text("Exceptions", "Rank: {} \nDate: {} \n{}".format(str(self.rank), time.ctime(), str(e)),
                                self.model_step)
        ...
        if dataset:
            ...
            for dataset_i, dataset in enumerate(dataset):
                ...
                if self.ema_model != None:
                    ...
                    # Log
                    if self.rank == 0 and writer is not None:
                        self.log_step(val_losses, val_metrics, {}, writer, step,
                                      os.path.join(tag + "-ema", str(dataset_i)))
    ...
    def evaluate(self, dataset_eval, eval_steps=None, verbose=0, recompute_metrics=False):
        ...
        for step, batch in enumerate(eval_iterator):
            ...
            inputs, targets = batch["inputs"], batch["targets"]
            _, _, name = inputs
            name = name.reshape(-1, *name.shape[2:])
            ...
            for index, n in enumerate(name):
                self.pred_result[n.item()] = {
                    # 'losses': batch_losses[index].detach().cpu(),
                    # 'metrics': batch_metrics[index].detach().cpu(),
                    'truths': batch_truths['wer'][index],
                    'preds': batch_preds['wer'][index],
                }
        ...
        if recompute_metrics:
            for key in epoch_metrics.keys():
                truths = ['{} {}'.format(i, j) for i, j in enumerate(epoch_truths[key])]
                preds = ['{} {}'.format(i, j) for i, j in enumerate(epoch_preds[key])]
                print(truths)
                print(self.pred_result)
                truth_path = '{}/res/truth_{}.txt'.format(self.my_settings['save_path'], self.epoch)
                pred_path = '{}/res/pred_{}.txt'.format(self.my_settings['save_path'], self.epoch)
                wer_path = '{}/res/wer_{}.txt'.format(self.my_settings['save_path'], self.epoch)
                f = open(truth_path, 'w+')
                f.write('\n'.join(truths))
                f.close()
                f = open(pred_path, 'w+')
                ...
                cmd = ('python ./metric/OpenEAT-main/tools/compute-wer.py --char=1 --v=1 {} {} > {}'
                       .format(truth_path, pred_path, wer_path))
                os.system(cmd)
    ...
    def swa(self, dataset, callback_path, start_epoch, end_epoch, epochs_list=None, update_steps=None, swa_type="equal",
            swa_decay=0.9, precision=torch.float32):
        ...
        if swa_type == "equal":
            ...
            swa_model = torch.optim.swa_utils.AveragedModel(self,
                                                            avg_fn=lambda averaged_model_parameter, model_parameter,
                                                                          num_averaged: (
                                                                                                    1 - swa_decay) * averaged_model_parameter + swa_decay * model_parameter)
        ...
        # Save Model
        if self.rank == 0:
            self.save(os.path.join(callback_path,
                                   "checkpoints_swa-{}-{}-{}.ckpt".format(swa_type, epochs_list[0], epochs_list[-1])),
